...
--- MyGame::Image#add_animation(table)
    アニメーション・パターンを追加します。
    ((|table|)) はラベル(アニメーションの識別子となる Symbol )をキーに
    パラメータの配列を引けるエントリのコレクションです
    (擬似キーワード引数風 Hash を受け取ることを想定しています)。
--- MyGame::Image#angle
...
--- MyGame::Image#start_animation(label, force_restart = false)
    ((|label|)) に関連付けられたアニメーションを開始します。
    そのアニメーションが既に実行中ならば、 ((|force_restart|)) が真でない限り
    何もしません。
--- MyGame::Image#stop_animation
    現在のアニメーションを停止します。
    (アニメーション中でなければ何も起こりません)
